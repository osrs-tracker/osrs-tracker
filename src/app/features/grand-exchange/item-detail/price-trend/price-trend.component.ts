...
export class PriceTrendComponent implements OnInit {
  ...
  getItemDetails() {
    return this.itemService.itemDetails(this.id).pipe(
  ...
  getData() {
    return this.itemService.itemGraph(this.id).pipe(
      map((response: { daily: any }) => Object.keys(response.daily).map((time: string) => ({
    ...
    this.priceTrendChart = new Chart(this.priceTrendCanvas.nativeElement, {
      ...
      options: {
        ...
        tooltips: {
          ...
          position: 'nearest',
          callbacks: {
            title: (tooltipItems, data) => new Date(tooltipItems[0].xLabel)
