...
export class XpService {
    ...
    insertInitialXpDatapoint(username: string, hiscore: Hiscore): Observable<Hiscore> {
        return this.httpClient
            .post(`${environment.API_OSRS_TRACKER}/xp/${username}/initialDatapoint`, { xpString: hiscore.srcString })
            .pipe(map(() => hiscore));
    }

    getXpFor(username: string, period: number = 14, offset: number = 0): Observable<Xp[]> {
        return this.httpClient
            .get<{ date: string; xpString: string }[]>(`${environment.API_OSRS_TRACKER}/xp/${username}/${period}`, {
